...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function restoreFolder() {
      try {
        const stored = (await window.indexedDB?.databases)
      ...
      try {
        ...
        for await (const entry of folderHandle.values()) {
          if (
            ...
            dateFolders++;
            for await (const fileEntry of (
              entry as FileSystemDirectoryHandle
            ).values()) {
              if (fileEntry.kind === "file") {
                const name = fileEntry.name.toLowerCase();
                if (name.startsWith("reminder-") && name.endsWith(".md"))
                  reminders++;
  ...
  return (
    <>
      ...
      <Separator />
      <p>Choose a local folder where your journal notes will be saved.</p>
      <Button
        onClick={pickFolder}
        className=""
      >
